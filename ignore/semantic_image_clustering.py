...
with tf.device("/GPU:0"):
    ...
    for batch_idx in tqdm(range(num_batches)):
        ...
        similarities = tf.linalg.matmul(
            current_batch, feature_vectors, transpose_b=True
        )
    ...
    labels = np.ones(shape=(x_data.shape[0]))  # error here in dummy labesl???
    # Compile the model.
    ...
    clustering_probs = clustering_model.predict(
        x_data, batch_size=batch_size, verbose=1
    )
